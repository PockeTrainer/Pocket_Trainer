...
function TodayRecord(){
    ...
    const get_data_from_sercer=async()=>{//선택한날의 음식들 정보를 다 가져옴
        ...
        await axios.get(`http://127.0.0.1:8000/diet/${tmp_date.getFullYear()+"-"+parseInt(tmp_date.getMonth()+1)+"-"+tmp_date.getDate()}/${id}`)//서버로 음식들 전송
            .then((res) => {
    ...
    const sendfood_to_server_update=(new_obj)=>{//추가해주기용 api
        ...
        new_obj.map(async(food,index)=>{
            await axios.post(`http://127.0.0.1:8000/diet/${tmp_date.getFullYear()+"-"+parseInt(tmp_date.getMonth()+1)+"-"+tmp_date.getDate()}/${id}`,
            {
    ...
    const sendfood_to_server_remove=(new_obj)=>{//지워주기용 api
        ...
        new_obj.map(async(food,index)=>{
            ...
            await axios.delete(`http://127.0.0.1:8000/diet/${tmp_date.getFullYear()+"-"+parseInt(tmp_date.getMonth()+1)+"-"+tmp_date.getDate()}/${id}`,
            {
                data:{ 
                    time:food.when_to_eat,
                    food_name:food.Info_from_api.DESC_KOR,
