...
function MainPage() {
    ...
    let [squart, changeSquart] = useState();
    ...
    const onClick = () => {
        axios.put("http://127.0.0.1:8000/api/user/testResult/2", {
            pushUp : pushUp,
            sitUp : sitUp,
            squart : squart
        })
        .then(res => {
            ...
            let upperBodyStrength = parseInt(res.data.upperBodyStrength);
            let stomachStrength = parseInt(res.data.stomachStrength);
            let lowerBodyStrength = parseInt(res.data.lowerBodyStrength);
            let totalInfo = (upperBodyStrength+stomachStrength+lowerBodyStrength) / 3

            console.log(typeof(upperBodyStrength))

            if (1 <= totalInfo && totalInfo <= 2) {
            ...
            } else if (3 <= totalInfo && totalInfo <= 4) {
